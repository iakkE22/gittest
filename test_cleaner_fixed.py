...
def test_single_text():
    ...
    if filter_result:
        ...
        if is_scenic:
            ...
            if extract_result:
                print(f"原始提取结果: {extract_result}")
                
                try:
                    # 清理返回结果中的markdown标记
                    clean_result = clean_json_result(extract_result)
                    print(f"\n清理后的结果: {clean_result}")
    ...
    try:
        ...
        for i, item in enumerate(test_data):
            ...
            try:
                ...
                # 显示提取的关键信息
                print(f"  适用人群: {keywords.get('适用人群', 'N/A')}")
                print(f"  写作风格: {keywords.get('写作风格', 'N/A')}")
                print(f"  景点名称: {keywords.get('景点名称', 'N/A')}")
                
            except json.JSONDecodeError as e:
                print(f"❌ 关键词提取结果格式错误: {e}")
                print(f"原始结果前200字符: {extract_result[:200]}...")
        ...
        if results:
            ...
            with open("test_output/test_results_fixed.json", 'w', encoding='utf-8') as f:
                json.dump(results, f, ensure_ascii=False, indent=2)
            print("测试结果已保存到 test_output/test_results_fixed.json")
